...
#' @details \code{RelistBibEntry} is only intended for use with
...
RelistBibEntry <- function(flesh, skeleton = NULL){
  ...
  res[[1L]] <- structure(flesh[seq_len(type.ind[1L]-1L)],
                         bibtype=as.character(flesh[type.ind[1L]]),
  ...
  if (N > 1L){
    for (i in 2L:N){
      ...
      res[[i]] <- structure(flesh[(key.ind[i-1L]+1L):(type.ind[i]-1L)],
                            bibtype= as.character(flesh[type.ind[i]]),
                            dateobj = dat,
