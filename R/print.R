...
#' @param .opts a list of formatting options from \code{\link{BibOptions}}.  Possible
#' options are
#' \itemize{
...
print.BibEntry <- function (x, .opts = list(), ...){
  ...
  sorting <- if (!length(.BibOptions$sorting))
               sorting <- switch(.BibOptions$bib.style, authoryear = 'nyt',
                                 alphabetic = 'anyt', draft = 'debug', 'nty')
