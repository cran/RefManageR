...
##     unloadNamespace("RefManageR")
##     library(RefManageR)
##     file.name <- system.file("Bib", "biblatexExamples.bib",
##                              package="RefManageR")
...
test_that("full name search", {
    ...
    expect_equal(length(bib[
        author="Mart\u00edn, Jacinto and S\u00e1nchez, Alberto"
                           ]), 1L)
...
test_that("search by key", {
    expect_equal(length(bib2[c("chen2013using", "carroll1978distributions")]),
                 2L)
})
...
test_that("search with bibtype and institution", {
    expect_equal(length(bib2[author = 'ruppert', bibtype = "report",
                             institution = "north carolina"]), 10L)
})
...
test_that("search excluding dates", {
    expect_equal(length(SearchBib(bib2, author='ruppert', date = "!1990/1999")),
                 59L)
})
...
test_that("three author search", {
    expect_equal(length(bib2[
        author="Carroll, R. J. and Simpson, D. G. and Ruppert, D."
                            ]), 5L)
})
...
##     len <- length(bib2[list(author='ruppert',bibtype="report",
